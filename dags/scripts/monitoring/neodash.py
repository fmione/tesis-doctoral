...
from neodb.model import Experiment
...
# cd dags/scripts/monitoring
# streamlit run neodash.py
...
if __name__ == "__main__":
    ...
    st.set_page_config(page_title="Knowledge graph interaction", layout="centered", initial_sidebar_state="expanded", menu_items={})

    with st.sidebar:
        st.header("Sections")
        st.markdown('###')
        st.write("Knowledge Graph monitoring")
        st.write("Historical experimental data")
        st.write("Metadata exploration")
        st.write("LLM Cypher querying")

    st.markdown("# Knowledge Graph monitoring")

    with open("style.css") as f:
        st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)

    # get types of measurement
    types, _ = db.cypher_query("MATCH (n:Measurement) RETURN DISTINCT n.type")

    dash_style = st.radio("Select graph type :", ["Each MBR separated", "Compare MBRs in one graph"])

    runID = int(st.number_input("Run Id", value=623))

    exp_nodes = Experiment.nodes.get(run_id=runID)
    mbr_nodes = exp_nodes.mbrs.order_by("exp_id").all()

    select_mbr = st.selectbox("Exp. Ids (MBRs)", mbr_nodes, format_func=lambda i: i.exp_id)
    select_type = st.selectbox("Measurement type", types, format_func=lambda i: i[0])

    measurements = select_mbr.measurement.filter(type=select_type[0])

    # measurements

    # mea_df = pd.DataFrame([i.__dict__ for i in measurements])
    measurement_df = pd.DataFrame([i.__dict__ for i in measurements]).sort_values(by=["time"])
    measurement_df["time_h"] = measurement_df["time"] / 3600

    st.markdown('#')
    # st.line_chart(measurement_df, x="time_h", y="value")
    st.altair_chart(
        alt.Chart(measurement_df).mark_line(point=True).encode(x="time_h", y="value").interactive(), 
